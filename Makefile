...
.PHONY: help install seed \
	...
	mp init-local \
	test smoke verify template clean
...
# 本机可被外部访问的 IPv4（公网优先，hairpin NAT 受限时自动降级 LAN）。
LOCAL_IP := $(shell bash $(ROOT_DIR)/scripts/local_ip.sh)
BACKEND_URL := $(shell bash $(ROOT_DIR)/scripts/local_ip.sh url 8000)
ADMIN_URL := $(shell bash $(ROOT_DIR)/scripts/local_ip.sh url 5173)
...
help:
	...
	@echo ""
	@echo "  make init-local       探测本机 IP 并写入前端 .env.local（miniprogram/admin-web）"
...
init-local:
	@echo "探测本机 IP 并写入前端 .env.local..."
	@bash "$(ROOT_DIR)/scripts/local_ip.sh" write "$(ROOT_DIR)"
...
admin:
	@if [ ! -f "$(ADMIN_DIR)/.env.local" ]; then \
		echo "[guard] admin-web/.env.local 缺失，自动跑 init-local 探测本机 IP"; \
		$(MAKE) -s init-local; \
	fi
...
mp:
	@if [ ! -f "$(MP_DIR)/.env.local" ]; then \
		echo "[guard] miniprogram/.env.local 缺失，自动跑 init-local 探测本机 IP"; \
		$(MAKE) -s init-local; \
	fi
	@if ! command -v npm >/dev/null 2>&1; then \
		echo "未找到 npm，请先安装 Node.js 和 npm"; \
...
verify: seed backend
	...
	for i in $$(seq 1 30); do \
		if curl -fsS --noproxy '*' "$(BACKEND_URL)/docs" >/dev/null 2>&1; then \
			READY=1; \
			echo "后端服务已就绪"; \
			break; \
		fi; \
